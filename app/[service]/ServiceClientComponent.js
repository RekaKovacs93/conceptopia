...
export default function ServiceClientComponent({ service }) {
  useEffect(() => {
    ...
    navtl.to(".descarr", { opacity: 1, duration: 0.5 });
  ...
  return (
    <main className="flex flex-col items-center bg-dark ">
      <div className="flex flex-col justify-center items-center mt-60 bg-dark min-h-screen">
          ...
          <h1 className="descarr opacity-0   hover:cursor-hover transition-transform pt-10 pb-20">
              <a href={`${service.url}#szolg`}><img width="40" height="40" src="https://img.icons8.com/FFEBCD/64/expand-arrow.png" alt="expand-arrow"/></a>
          </h1>                
        ...
        <div className="gradient text-3xl text-center font py-20" id="szolg">
        <h1>Miért fontos a professzionális {service.title.toLowerCase()} a vállalkozásod számára?</h1>
        <div className=" w-screen flex flex-wrap gap-5 items-center justify-center md:px-40 px-10 mb-20 pt-20 ">
       
                {service.features.map((feature, index) => (            
                    <div key={index} className="md:h-56 w-96 rounded-2xl">
                ...
                <div id="desc" className="md:opacity-0 opacity-100">
                  ...
                  <p className="text-light text-base text-center px-5 pt-2">{feature.desc}</p>
                  </div>
                </div>
                {/* </Link> */}
          </div>
